...
# for all emotion classes: anger, fear, joy, love, sad and suprise.
import pickle
import numpy as np
import warnings
...
# ⚠️ NOTE: This is now a scikit-learn model saved with pickle, NOT a Keras model!
...
#  Test prediction of the New Neural Network model
y_pred_nn = load_model.predict(user_dense)[0]  # MLPClassifier returns single prediction, not array

# Convert numeric prediction back to emotion label
predicted_emotion = label_encoder.inverse_transform([y_pred_nn])[0]

# Get prediction probabilities
probabilities = load_model.predict_proba(user_dense)[0]
